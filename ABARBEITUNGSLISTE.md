...
- ✅ **BESTÄTIGT (fixen):** **K1** (MWSt 7.7→8.1/3.8 = Fakt · *erledigt v0.9.174*), **K2** (datenqualitaet „Reviews vs. Kalender" misst nur Kalender-Präsenz = immer 100 %, Label log → umbenannt · *erledigt v0.9.174*), **K9** (compEst-Zahl ohne Tier), **K10** (investor Rent-Sensitivität nutzt Kauf-Konstanten), **K11** (Markt-Pool-Dedup: Zähler deduped / Nenner roh). **K8** (Homegate: `'2.5'.replace('.','')`→`'25'` = „25 Zimmer" — Bug real; Homegate-Query-Format vor Fix verifizieren).
