...
## [0.9.128] — 2026-06-14 — Umsatz je Kategorie: Breakdown-Charts umschaltbar Auslastung % ↔ Umsatz CHF/Mt

Anwender-Feedback (Adrian zeigte das Tool herum): KPIs top, schnell verständlich — Wunsch: Umsatz auch auf Kategorie-Ebene (Zimmer, Superhost, Kapazität …) sichtbar. **Umgesetzt:** Umschalter über den 6 Breakdown-Charts — **„Auslastung %" ↔ „Umsatz CHF/Mt"** (`METRIC`). Im Umsatz-Modus zeigt jeder Balken den **Median-Monatsumsatz** der Kategorie = `Preis × Auslastung(@H) × 30 Nächte` (`umsatz()`/`medUmsatz()`, Median robust gegen Ausreisser), Balkenbreite relativ zur grössten Kategorie; die Chart-Titel wechseln „Auslastung → Umsatz" (`.mword`). Ehrliche Tier-Zeile: „🟡 Median-Umsatz/Mt = Preis × Auslastung(30T) × 30 N · Brutto (vor Kosten), Modell". Reagiert auf alle Filter + Profi-Toggle + Horizont wie die Auslastung. **Verifiziert (Browser, Kriens, 0 Fehler):** Kapazität — 2P CHF 1'617 / 4P CHF 2'988 / **5+P CHF 5'655** Median/Mt (vs. Auslastung 67/74/84 %); Titel-Wechsel + relative Balken + „kein Angebot" weiter korrekt. Bewusst Brutto-Umsatz (Top-Line), nicht Netto — der volle Netto-Rechner bleibt der Geld-Fluss-Block pro Inserat.

## [0.9.127] — 2026-06-14 — „Nur aktive Profis"-Filter (aus dem Kalender, nicht aus Bewertungen)

Adrians Punkt: Profi vom Rest trennen — Kriterien Buchungs-Aktivität + Bewertungen, aber als zuschaltbarer Filter; und: der Kalender zeigt echtes Geschäft auch ohne Airbnb-Reviews (Channel-Sync mit Booking etc., Review-Verzögerung). **Umgesetzt kalender-zuerst (plattform-unabhängig):** `cal_managed` (Block-Heuristik: aktiv vermietet vs. host-blockiert/privat) + `cal_occ_raw_pct`/`cal_longest_block_days` werden jetzt von `compdata.py` ins Cockpit durchgereicht. Neuer Toggle **„⭐ nur aktive Profis"** (`F.profi`, neben „nur Gemeinde"): `isProfi(l)` = **Kalender zeigt echtes Geschäft** (`cal_managed!==false`, also nicht host-blockiert) **UND occ@30 ≥ 40 %** (~12 belegte Nächte/Monat) **UND ≥ 30 Bewertungen** (Track-Record). Schwellen transparent im UI angezeigt (Hinweis-Zeile „N von M aktive Profis (Kalender: occ@30 ≥ 40 % · ≥ 30 Bew · nicht host-blockiert)"), kombiniert mit allen anderen Filtern. **Verifiziert (Browser, Kriens, 0 Fehler):** 4 Inserate als host-blockiert erkannt + ausgeschlossen, Toggle 66 → 30 aktive Profis (Emanuel 551 Bew/80 %, Franz & Helene 373/83 %…). **Bewusst NICHT review-basiert:** Review-Aktualität (Bewertung im Vormonat) braucht einzelne Review-Daten (StayPdpReviews-Endpoint, offen) und ist ohnehin schwach (Reviews verzögert, Booking-Buchungen erzeugen keine Airbnb-Review). Das schärfste Profi-Signal wird die **gemessene Buchungs-Velocity** aus der Pickup-Zeitreihe, sobald sie wächst. cal_managed für die anderen 5 Gemeinden kommt automatisch mit dem nächsten Lauf.

## [0.9.126] — 2026-06-14 — Mehrfachauswahl pro Filter-Block + „kein Angebot" sichtbar

Adrians zwei Punkte: (1) im selben Block mehrere Kästchen wählbar; (2) ein „−" heisst kein Angebot in der Kategorie — das soll man sehen. **Mehrfachauswahl:** die 6 Filter-Dimensionen (`F.group/rating/vol/type/cap/rooms`) sind jetzt **Sets** statt Einzelwert — innerhalb eines Blocks OR (z.B. „2P" + „3P" = kleine Einheiten), zwischen Blöcken weiter AND. `matchDim()` prüft Mengen-Zugehörigkeit, `toggle()` fügt hinzu/entfernt, jeder gewählte Balken kriegt einen eigenen Chip, ausgewählte Zeilen dezent gold hinterlegt (`.bar-row.on`), `near` bleibt Boolean; Prognose/Pickup-„anyFilter" auf `.size` umgestellt. **„Kein Angebot" sichtbar:** Kategorien mit 0 Inseraten zeigen „<span>– kein Angebot</span>" + Zähler 0, sind **ausgegraut (`.bar-row.empty`) und nicht klickbar** (man kann nicht auf eine leere Menge filtern). **Verifiziert (Browser, 0 Fehler):** Kapazität 2P (36 Inserate) → +3P (41, OR), beide Balken markiert + zwei Chips; „4+ Zi"-Leerzeile grau/„kein Angebot"/nicht klickbar. Bestehende Single-Klick-Bedienung bleibt (ein Balken = ein Filter), „Alle" leert die Gruppe.

## [0.9.125] — 2026-06-14 — Buchungs-Momentum folgt der Auswahl (Host anklicken zeigt SEINE Buchungen)

Adrians Wunsch: „würde es gehen, wenn ich Lukas anklicke und sehe, was bei ihm dazugekommen ist? Jetzt ist es nicht verbunden, der Filter geht nicht." — die Pickup-Kachel war markt-weit und reagierte nicht auf Auswahl/Filter. Jetzt **verbunden wie der Rest des Cockpits.** `pickup.py` schreibt zusätzlich `per_listing` in die JSON (pro Inserat mit Bewegung: `{nb, fr, newly[]}` — nur geänderte Inserate, kompakt). `renderPickup(cur)` aggregiert je nach Geltungsbereich: **gewähltes Inserat** (SEL → „für <Host> ↗", seine Buchungen + Vorlaufzeit) > **gefilterte Auswahl** („für deine Auswahl (N)", Summe) > **Markt** (Default). Verdrahtet in `render()` + Tabellen-Klick (`leadBucketsJS` für die Vorlaufzeit-Verteilung je Scope). Fussnote neu: „Folgt deiner Auswahl: Host anklicken zeigt seine Buchungen, Filter zeigt deren Summe." **Verifiziert (Browser, 0 Fehler):** Markt (66 Inserate, +13) → Host Ariser (+1, 1 neu gebucht) → Filter Superhost (37 Inserate, −5 netto = mehr Stornos); kein Eintrag/SEL ohne Bewegung → ehrlicher Hinweis „keine Bewegung (oder neu im Set → ab morgen vergleichbar)". Hinweis Lukas: heute erst zurückgeholt (nur im 06-14-Snapshot) → sein per-Inserat-Pickup zündet ab dem 06-15-Lauf (in beiden Snapshots).

## [0.9.124] — 2026-06-14 — Persistenz: bekannte Inserate nicht verlieren (Lukas zurückgeholt)

Adrians Befund: „was ist mit meinem Lukas passiert, den wir gestern so ausgiebig angeschaut haben?" — und er war weg. **Diagnose:** Lukas (Kriens, rooms/36406870, 144 Bew, 6-Pers, ~90 % belegt) war in `24b01e6` (Einzel-Fenster) drin, fiel aber im Mehr-Fenster-Re-Scrape (`65c0116`) raus — obwohl 53 dazukamen. Kehrseite des v0.9.120-Fixes: der Scrape **ersetzt** die Liste, statt zu unionen → gerade die **stärksten, dauergebuchten** Inserate, die keine Suche re-surfaced (zu ausgebucht), fallen raus. Lukas lebt (Kalender per ID abrufbar, 183 Tage). **Struktureller Fix:** Persistenz in `fetch_airbnb_free.py` — bekannte in-radius-`listingId`s aus dem letzten Lauf werden per ID unioniert (`carried_from_prev`), der Kalender (geht per ID, ohne Suche) refresht sie; nur echt delistete verschwinden. So kann kein bekanntes Inserat mehr durch Such-Ranking verloren gehen. **Recovery Lukas:** die in der Mehr-Fenster-Umstellung verlorenen Kriens-Inserate aus `24b01e6` zurückgeholt (Lukas + Gabriela + Viviana + Walter Anton), competitors entdoppelt (105→83). Kriens-Cockpit 62→**66, Lukas drin** (144 Bew, 6 P, CHF 598, occ30 67 %, in Gemeinde). **Offen:** Emmen/Baden ggf. gleiche Recovery (Meggen/Horw/Ebikon wurden geo-frisch gescrapt); die Persistenz schützt ab jetzt alle Gemeinden automatisch. (Nebenbefund: Mehr-Fenster-Union erzeugt vereinzelt Duplikate — von compdata/dedup abgefangen, sauberer Fix später.)

## [0.9.123] — 2026-06-14 — Such-Fläche aus der Gemeindegrenze (Adrians „Map aktivieren") — kein geratener Radius mehr

Adrians Punkt: „was machst du mit zu weiten Radien — man kann ja die Map aktivieren, dann stimmt alles." Genau. Statt die Map-Bounds aus einem geratenen Radius-Kreis zu bilden, kommen sie jetzt aus der **echten Gemeindegrenze** — die `boundary-<m>.geojson` haben wir je Gemeinde schon, ihr Bounding-Rechteck (+1 km Puffer) ist die präzise Such-Fläche. Neue `boundary_bbox(market)` in `fetch_airbnb_free.py`: liest die Grenze, gibt ne/sw-Bounds + effektiven Zoom; `run()` nutzt sie bevorzugt (Fallback = Radius-Kreis, wenn keine Grenze da), Logzeile „Such-Flaeche: Gemeindegrenze+1km". **Selbstkorrigierend pro Gemeinde** — kein per-Gemeinde-Radius-Tuning mehr nötig, die Grenze ist die Wahrheit. **Abnahme (Meggen via Grenz-Bounds):** in-Gemeinde **10** (≥ Radius-4-Ergebnis), echte Perlen drin (Raphie 342 Bew, Lorenzo 332, Lukas Martin 154). Horw 14→15, Ebikon 12→13 (mit nachgezogenen Radien re-gescrapt; künftige -Full-Läufe nutzen automatisch die Grenz-Bounds). Die manuellen Radien (v0.9.122) bleiben als Fallback + für `enrich_geo`, sind für die Suche aber nicht mehr kritisch. Gilt für ALLE Gemeinden mit Grenze über den einen Code-Pfad.

## [0.9.122] — 2026-06-14 — Meggen-Geo gelöst: Radien an echte Gemeindegrösse angepasst (war zu weit, zog Luzern rein)

Adrians Befund: „Meggen ist eine grosse Gemeinde, vielleicht ist sonst was nicht okay" — und er hatte recht. **Diagnose:** Marktzentrum + Gemeindegrenze von Meggen sind korrekt, aber der Radius war pauschal **8 km bei nur ~3.2 km echter Grenz-Reichweite (2.5× zu weit)** → die Suche reichte bis nach Luzern (3 km weg), die Mehr-Fenster-Discovery zog 101 Luzern-Inserate, nur **2 lagen wirklich in Meggen**; die echten Meggen-Inserate wurden aus den Such-Resultaten verdrängt. Systematisch geprüft: alle Fokus-Gemeinden hatten pauschal 8 km — Meggen 2.5×, Ebikon 1.8×, Horw 1.7× zu weit (Kriens/Emmen/Baden ≤1.4× = ok). **Fix:** `market-centers.json` Radien an Grenz-Reichweite + ~1 km Puffer ausgerichtet — Meggen 8→4, Horw 8→6, Ebikon 8→6. **Abnahme (Meggen, Radius 4):** in-Gemeinde **2 → 9**, die echten Meggen-Inserate sind zurück, inkl. **Raphie (342 Bewertungen, 93 % Auslastung)** und **Lorenzo (332 Bew)** — Top-Performer, die im 8-km-Luzern-Rauschen untergingen; 5 der ursprünglichen 8 wieder da. Cockpit n 63→38 (dafür Meggen-relevant statt Agglo-Müll). **Nebenfix:** `fetch_airbnb_free.py --force` umgeht den Drift-Guard für bewusste Parameter-Änderungen (Radius-Recalibration bringt legitim weniger Inserate, soll nicht als Degradation blockiert werden). Horw+Ebikon mit neuem Radius nachgezogen. **Lehre/offen:** Radius gehört langfristig aus der Gemeindegrenze abgeleitet (boundary_reach × Puffer), statt pauschal — sonst blutet jede agglo-nahe Kleingemeinde.

## [0.9.121] — 2026-06-14 — Erster Auto-Lauf live + Robustheit: Drift-Guard & Lock gegen Daten-Degradation

**Erster geplanter 06:00-Lauf gefeuert (automatisch, 14.06. 06:09):** Interactive-Logon + Auto-`commit`+`push` funktionieren (Commit `b1272c4` ohne Zutun in GitHub). **Erster echter Pickup live** (2 Snapshot-Tage): Emmen +39 / Ebikon +28 / Kriens +13 / Horw +11 Nächte netto seit gestern — gemessene Buchungen aus dem Kalender-Diff, Cockpit-Kachel zeigt es. **Aber ok=4 fail=2:** Baden+Meggen scheiterten, weil `competitors.json` zwischendurch auf einen degradierten Mini-Scrape zurückfiel (Kriens 97→10, Meggen 101→8 Inserate, 0 PDP) — die zuletzt laufenden Baden/Meggen fanden 0 anreicherte Inserate, der compdata-Guard stoppte korrekt (guter Stand blieb). Kein Reset im reflog → ein degradierter Scrape (vermutl. paralleler/rate-limitierter Lauf) hatte die guten Daten überschrieben. **Behoben + gehärtet:** (1) `competitors.json` aus `65c0116` wiederhergestellt, Baden/Meggen nachgebaut → 6/6 mit Snapshot 06-14, lückenlos. (2) **Drift-Guard** in `fetch_airbnb_free.py`: bricht ein Scrape ggü. dem letzten guten Stand um >50 % ein (und war der ≥20), wird `competitors.json` NICHT überschrieben (`raise SystemExit(2)`) — ein Rate-Limit/Block kann gute Daten nie mehr zerstören. (3) **Lock** in `daily_cockpit.ps1`: kein paralleler Lauf (Lockfile in `data/raw/`, 90-min-Stale-Schutz) — verhindert das Rennen zweier Scrapes. **Offen:** Meggen-Geo (Mehr-Fenster fand 101 Agglo-Inserate, aber nur 2 in Meggens kleiner Grenze — die ursprünglichen ~8 echten Meggen-Inserate wurden verdrängt; für Kleinstgemeinden Radius/Discovery-Ranking prüfen).

## [0.9.120] — 2026-06-13 — Mehr-Fenster-Discovery im Scraper (Contract G umgesetzt) — die ausgebuchten Top-Inserate sichtbar

Umsetzung von `scraper-contract.md` Abschnitt G (v0.9.119-Doc) im Code, gilt damit für **alle Gemeinden und alle künftigen Läufe** über den einen Code-Pfad. **Ursache des Monique-Fehlers:** `fetch_airbnb_free.py` suchte nur EIN Zukunfts-Fenster (+42 T) — die Airbnb-Suche zeigt nur *freie* Inserate, also waren die beliebten, oft ausgebuchten (Guest Favorites) systematisch unsichtbar. **Fix (additiv, risikoarm):** Referenz-Fenster (+42, treibt Preis/Signatur) bleibt unangetastet; danach scannt `run()` zusätzlich `DISCOVERY_EXTRA_OFFSETS = [21, 90, 150]` Tage (kurzer 3-Nächte-Probe-Aufenthalt = max. Verfügbarkeits-Oberfläche) und unioniert die `listingId`s; Herkunft je Inserat in `discovered_window_offset`, genutzte Fenster in `discovery_windows` der Signatur. **Abnahme-Test bestanden:** Ebikon re-gescrapt → **44 → 93 eindeutige Inserate (+49 zuvor unsichtbare)**, darunter Moniques „Sunny apartment near Lucerne" (id 14990297, 151 Bew, 5.0, gefunden im +90-T-Fenster = September, wo der Screenshot Verfügbarkeit zeigte); 100+-Bewertungen-Inserate im Cockpit **4 → 20**. Nach pdp_enrich + compdata: Ebikon-Cockpit **25 → 63 Inserate**, Monique drin mit occ30=90 % (in Gemeinde), Snapshot 2316 → 5928 belegte Kalendertage. **Folgefehler mitgefixt:** `daily_cockpit.ps1 -Full` übergab den Markt klein geschrieben an `fetch_airbnb_free` (`ebikon`), aber `market_center`/`precise_query` sind case-sensitiv (`Ebikon` ja, `ebikon` nein) → hätte beim Wochen-Re-Scrape die Geo-Bindung verloren; jetzt `--market $m` (gross). **Offen:** die anderen 5 Gemeinden mit dem neuen Code re-scrapen (folgt; der wöchentliche `-Full`-Lauf macht es künftig automatisch).

## [0.9.119] — 2026-06-13 — Buchungs-Momentum-Kachel im Cockpit (zündet ab dem 2. Snapshot)

Vorbereitet, damit die Pickup-Anzeige morgen nach dem 06:00-Lauf automatisch lebt: neue Kachel „Buchungs-Momentum" oben im Cockpit (nach KPIs/Filter-Chips), liest `data/cockpit-<m>-pickup.json` (vom Tageslauf via `pickup.py` erzeugt). **Heute (Datei fehlt noch): ehrlicher Wartezustand** „⏳ wird live, sobald der 2. Tages-Snapshot vorliegt … das erste wirklich gemessene Nachfrage-Signal, nicht modelliert" — kein Fake. **Sobald die Datei da ist:** grosse Netto-Zahl (+X Nächte, Velocity /Tag, grün/rot), Aufschlüsselung (neu gebucht / frei geworden / Inserate verglichen / prev→curr-Datum / neu-im-Markt), **Vorlaufzeit-Balken** (0–14 / 15–30 / 31–60 / 61+ T proportional) und 🟢-Methodenzeile (gemessen aus dem Kalender-Diff, markt-weit, folgt nicht den Filtern). Spezialfall „keine Bewegung" sauber abgefangen. `load()` holt die Pickup-JSON (404 → Wartezustand), `renderPickup()` in `try/catch` gekapselt (bricht nie den Rest). **Verifiziert (Browser, 0 Fehler):** Wartezustand zeigt korrekt; mit synthetischem Pickup-JSON die Live-Kachel geprüft (+11 Nächte netto grün, 14 neu/3 frei, Balken proportional 6/4/3/1 = 100/67/50/17 %), synthetisches File danach gelöscht (nicht committet); Wartezustand kehrt zurück, Tabelle 26 Zeilen intakt. (Reale Pickup-JSONs werden vom Tageslauf-`git add cockpit-*.json` automatisch committet.)

## [0.9.118] — 2026-06-13 — Inserate-Tabelle: sortierbar, klarer Airbnb-Link, entzerrte Host-Spalte

Adrians UI-Wünsche an die Tabelle: (1) linke Seite besser beschriftet/entzerrt, (2) sichtbar machen, wo man für den Airbnb-Link klickt, (3) Spalten oben nach klein/gross sortierbar. **Umgesetzt (renderTable datengetrieben):** (1) **Sortierbare Spalten** — Spaltentitel klicken sortiert; gleiche Spalte erneut = Richtung kippt (▼ gross zuerst / ▲ klein zuerst, aktiver Header gold + Pfeil), numerisch default absteigend, Host A→Z; Zustand in localStorage `cockpit_sort`. Sortierschlüssel je Spalte (Bewertungen/Sterne/Velocity/Pers/Tenure/Distanz/Portfolio/Preis/Gast/Host-Netto/Auslastung + Watchlist + Host-Name). (2) **Klarer Airbnb-Link** — Host-Name als sichtbarer blauer Link mit gestricheltem Unterstrich + „↗", Tooltip „Auf Airbnb öffnen"; plus **Legende** über der Tabelle, die die drei Klick-Aktionen erklärt (Host-Name ↗ = Airbnb · Spaltentitel = sortieren · Zeile = Geld-Rechner). (3) **Entzerrt** — Host-Spalte von minmax(108px) auf minmax(160px,2fr) (real ~271px statt gequetscht), kryptische Header mit Klartext-Tooltips (Bew. = Anzahl Bewertungen, Bew./Mt = Buchungs-Velocity, Preis/N = Host-Listenpreis usw.), Header sticky beim vertikalen Scroll, min-width 900→1000. **Verifiziert (Browser, 0 Fehler):** 13 Grid-Spalten intakt, Default Bew. ▼ (383/361/164), Klick→▲ (klein zuerst), Preis ▼ = 598 zuoberst, Host ▲ = A→Z, aktiver Header markiert, Airbnb-Link sichtbar mit echtem rooms-Link; Zeilen-Klick (Geld-Rechner) + ★-Watchlist unverändert.

## [0.9.117] — 2026-06-13 — Horw + Ebikon: Saison-Prognose via Regional-Proxy (BFS hat keine eigenen Daten)

Aufgedröselt: Horw + Ebikon haben **gar keine BFS-Hoteldaten** — die HESTA-Tabelle px-x-1003020000_201 enthält nur 186 Gemeinden, im Kanton Luzern nur Emmen/Kriens/Luzern/Meggen/Weggis/Vitznau (BFS-Metadaten direkt geprüft, Codes 1058/1054 nicht in der Tabelle → zu wenige/keine Hotelbetriebe). Sie können also **nie** eine eigene BFS-Saison kriegen. Ehrliche Lösung (additiv, ohne die index.html-Engine anzufassen): **klar gelabelter Regional-Proxy** — die normierte Saison-FORM des nächsten vergleichbaren Nachbarn mit Daten. `data/cockpit-season-proxy.json` (`{horw:Kriens, ebikon:Kriens}`, Kriens grenzt direkt an, gleiche Agglo/Grösse; die Saison-Wellen sind regional, nicht gemeindespezifisch — dieselbe Logik wie „Hotel≈STR-Saison", einen Schritt weiter). `cockpit.html` `load()` konsultiert die Proxy-Map nur wenn die Gemeinde kein eigenes Profil in market-facts hat, lädt dann die Saison-Form + occ_band-Floor des Proxy-Markts und setzt `_seasonal.proxy`. **Ehrlich getrennt:** das gemessene **Niveau bleibt von der Gemeinde selbst** (Pool-Median ihres eigenen Airbnb-Pools), nur die **Verteilung übers Jahr** ist geliehen — im UI sichtbar als „⚠ Saison-Proxy Kriens" (Untertitel) + Fussnoten-Erklärung. **Verifiziert (Browser, 0 Fehler):** Horw rechnet (eigenes Niveau 58 % × Kriens-Form, Jun 73/Jul 81), Ebikon rechnet (eigenes, niedrigeres Niveau 53 %, Jun 67/Jul 75 — beweist: Niveau ist je Gemeinde verschieden, nur Form geteilt), beide mit Proxy-Label; Kriens zeigt **kein** Proxy-Label (eigene Daten); Tabellen 23/25 Zeilen intakt. Damit haben alle 6 Cockpit-Gemeinden eine Prognose.

## [0.9.116] — 2026-06-13 — Pickup-Analyse: echte Buchungen aus dem Snapshot-Diff (der Payoff der Retention)

`tools/pickup.py` vergleicht zwei datierte Snapshots einer Gemeinde und macht die **echten Buchungen** sichtbar — genau das, was ein einzelner Snapshot nie kann: ein Tag, der von frei→belegt springt, ist eine Buchung; belegt→frei ein Storno/Block-Freigabe. Pro Lauf: **Netto-Pickup** (Nächte, +/Tag = Velocity), **Vorlaufzeit-Verteilung** der Neu-Buchungen (0–14 / 15–30 / 31–60 / 61+ T — zeigt, wo die Nachfrage landet), Inserate verglichen/neu/verschwunden. Vergleicht nur Inserate, die in **beiden** Snapshots vorkommen (matcht per id). Ehrlich kommentiert: frei→belegt ist sehr wahrscheinlich Buchung (v.a. nahe Tage = stärkstes Signal), kann aber host-Block sein — über viele Tage wird das Muster eindeutig. **In den Tageslauf verdrahtet:** `daily_cockpit.ps1` ruft nach jedem Lauf `pickup.py --all --json` → schreibt `data/cockpit-<m>-pickup.json` (vom bestehenden `cockpit-*.json`-git-add miterfasst, landet off-machine). **Verifiziert:** Diff-Logik per Self-Test gegen synthetische Snapshots korrekt (newly/freed/net/n_common = 1/1/0/2, Vorlaufzeit-Bucket korrekt); `--all` zeigt heute graziös „erst 1 Snapshot — kommt mit dem nächsten Tageslauf". **Zündet morgen automatisch:** nach dem 06:00-Lauf hat jede Gemeinde 2 Tage → erster echter Pickup. Nebenbefund (für nächsten Schritt): Horw + Ebikon haben keinen direkten `market_to_bfs`-Eintrag → BFS-Anbindung ihrer Prognose ist ein eigener Schritt.

## [0.9.115] — 2026-06-13 — Täglicher Cockpit-Runner (Free-Pipeline + Auto-Commit/Push) als Windows-Aufgabe

Der bestehende `run_focus_daily.ps1` nutzte die **bezahlte** BrightData-Pipeline (`fetch_airbnb.py`, API-Key). Neuer **`tools/daily_cockpit.ps1`** fährt den **Gratis-Cockpit-Weg** vollautomatisch, state of the art: pro Fokus-Gemeinde (`tools/cockpit_communes.txt`: Kriens/Emmen/Horw/Ebikon/Baden/Meggen) den öffentlichen Airbnb-Kalender frisch holen → `compdata.py` schreibt Serving-JSON + datierten Snapshot, dann **`git add data/snapshots data/cockpit-*.json` → commit → push** (Daten landen täglich off-machine in GitHub). **Robustheit:** (1) **Selbstheilung** — scheitert eine Gemeinde, laufen die anderen weiter (per-Gemeinde try/catch); ein Leer-Scrape (0 Inserate) überschreibt den letzten guten Stand NICHT (neuer Guard in `compdata.py`). (2) **Push-Ausfall ist nicht-fatal** — Commit bleibt lokal, landet beim nächsten erfolgreichen Push, Daten nie verloren. (3) Transcript-Log pro Tag in `data/raw/` (gitignored). (4) Coverage-Report nach jedem Lauf. Flags: `-Full` (zusätzlich Suche neu scrapen + PDP anreichern, wöchentlich sinnvoll), `-NoPush` (Testlauf). **`tools/register_daily_task.ps1`** registriert die Windows-Aufgabe „SwissSTR-Cockpit-Daily" (täglich 06:00, `StartWhenAvailable` holt verpasste Läufe nach, `WakeToRun`, laptop-/akku-tauglich, 2× Retry). **LogonType Interactive** (läuft als eingeloggter Benutzer) — S4U („ob eingeloggt oder nicht") brauchte Admin (Register-ScheduledTask → Zugriff verweigert ohne Elevation), Interactive geht ohne Admin und hat zudem die Git-Credentials zur Hand. Heisst: läuft beim 06:00-Trigger wenn eingeloggt, sonst beim nächsten Login (StartWhenAvailable). **End-to-End verifiziert:** Testlauf 6/6 Gemeinden ok in 109 s, Snapshots geschrieben, `git commit + push` erfolgreich (Commit `ef0fb72` in GitHub); Aufgabe registriert, Status Ready, nächster Lauf 14.06. 06:00.
