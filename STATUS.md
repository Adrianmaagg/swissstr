...
> **Dies ist der EINE verbindliche Projektstand.** Bei Widerspruch mit `README.md`, `CHANGELOG.md` oder alten Docs in `docs/_legacy/` gilt **diese Datei**. Letzte Pflege: 2026-06-19 (v0.9.222 — **Marktanalyse vertieft: Wettbewerbs-Raster im Cockpit** (Adrians Kundensicht-Pricing §4, Preis × Klasse mit Lücke/Preismacht) **+ Mindestnächte-Erfassung** (§5, aus dem Free-Kalender), browser-verifiziert. Davor v0.9.211–220: Modularisierung 7/7 + Kapselung, Daten-Vertrauen/Dogfood/Konkurrenz, 3-Kohorten gelöst, „woher?"-Beleg, Strategie-Brief. Offen: positionierung-Schärfung, Mindestnächte-Scrape, siehe §7).
...
neuer Scrape**. **✅ Nachgezogen (v0.9.222): Mindestnächte (§5)** — `fetch_airbnb_free.fetch_calendar`
behält jetzt `minNights` je Tag, `compdata` rechnet den Median buchbarer Tage (`min_nights`), das Raster
zeigt „min N" je Zelle (verifiziert auf Walchwil; aktiviert sich pro Markt mit dem nächsten Tages-Scrape).
**Befund:** der Free-Kalender liefert `minNights` gratis, aber **keinen Tagespreis** (null). **Noch offen:**
„not_released" (§2) + Kalender-Tiefe (Pfeiler 2) = letzter-freier-Tag aggregieren. „5+P" bündelt Grossobjekte.
...
- **Version:** v0.9.222 (`CHANGELOG.md` = volle Historie, `docs/` = Methodik-Specs · `docs/pricing-cockpit-methodik.md` = Raster/Pricing · `docs/review-brillen.md` = Review-Personas)
